    ...
    <div class="max-w-3xl w-full px-4 sm:px-8">
      ...
      <div class="flex items-center">
        <div class="flex-1 pr-3 sm:pr-6">
          <input type="text" v-model="searchValue" placeholder="请输入小说名称..." class="appearance-none block w-full py-3 px-4 leading-none sm:leading-normal border border-gray-100 placeholder-gray-500 rounded-lg focus:outline-none focus:shadow-outline-blue focus:z-10 sm:px-5 text-sm sm:text-lg" @keyup.enter="search">
        </div>
        <button class="inline-flex items-center px-4 border border-transparent font-medium rounded-lg text-white bg-gray-800 hover:bg-gray-700 focus:outline-none focus:border-gray-900 active:bg-gray-900 focus:shadow-outline-gray transition duration-150 ease-in-out py-3 sm:px-6 leading-none sm:leading-normal text-sm sm:text-lg" @click="search">
